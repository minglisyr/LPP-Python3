...
#    self.w.pack(expand=YES,fill=BOTH)
    ...
    def mouse_rotate(self, xnew, ynew, xold, yold):
        ...
        vold[2] = self.rtrack * self.rtrack - \
            vold[0] * vold[0] - vold[1] * vold[1]
        if vold[2] < 0:
            return
        vold[2] = sqrt(vold[2])
        vold = vecnorm(vold)
        ...
        vnew[2] = self.rtrack * self.rtrack - \
            vnew[0] * vnew[0] - vnew[1] * vnew[1]
        if vnew[2] < 0:
            return
        vnew[2] = sqrt(vnew[2])
        vnew = vecnorm(vnew)
    ...
    def setview(self):
        ...
        self.xto = self.center[0] - xfactor * \
            self.right[0] - yfactor * self.up[0]
        self.yto = self.center[1] - xfactor * \
            self.right[1] - yfactor * self.up[1]
        self.zto = self.center[2] - xfactor * \
            self.right[2] - yfactor * self.up[2]
    ...
    def redraw(self, o):
        ...
        glLoadIdentity()
        if self.orthoflag:
            glOrtho(-0.25 * self.eye, 0.25 * self.eye, -0.25 * self.eye, 0.25 * \
                    self.eye, self.eye - 2 * self.distance, self.eye + 2 * self.distance)
        else:
                    ...
                    for bond in self.bonddraw:
                        ...
                        glMaterialfv(
                            GL_FRONT_AND_BACK, GL_EMISSION, [
                                red, green, blue, 1.0])
                        glMaterialf(
                            GL_FRONT_AND_BACK, GL_SHININESS, self.shiny)
                        obj = gluNewQuadric()
                        gluCylinder(
                            obj, rad, rad, bond[10], self.nsides, self.nsides)
                        glPopMatrix()
