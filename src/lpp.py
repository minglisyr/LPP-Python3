...
class lpp:
    ...
    def __init__(self, *list, **kwargs):
        ...
        if "--cpunum" in kwargs:
            try:
                if int(kwargs["--cpunum"]
                       ) > 0 and int(kwargs["--cpunum"]) <= self.cpunum:
        ...
        if listlen == 1 and self.overwrite == False:
            raise Exception(
                "Cannot process single dump files with --no-overwrite.")
        ...
        while i < len(dumpInput):
            ...
            if self.output:
                print("calculating chunks", i + 1, "-",
                      min(i + self.cpunum, numberOfRuns), "of", numberOfRuns)
...
def lppWorker(input):
    ...
    if overwrite:
        ...
        for f in flist:
            ...
            filename, file_bb, file_walls = vtk.generateFilename(granName, [
                                                                 time], 0)
...
def printHelp():
    ...
    print(
        "--cpunum    : sets the number of processes to start, default (and maximum)",
