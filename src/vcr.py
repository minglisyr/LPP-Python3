...
class vcr:
    ...
    def __init__(self, *views):
        ...
        if len(views) == 0:
            raise Exception("must have at least one GL viewer")
        self.viewlist = []
        for view in views:
        ...
        self.button_save = Checkbutton(
            frame6, text="SaveAll", command=self.saveall)
    ...
    def loop(self):
        ...
        if self.saveflag:
            for view in self.viewlist:
                time, natoms = view.display(self.index)
            self.saveloop(1)
    ...
    def saveloop(self, flag):
        if flag == -1:
            ...
            return
        if flag == 0:
            self.fileroot = self.entry_file.get()

        if self.index < 10:
            file = self.fileroot + "000" + str(self.index)
        elif self.index < 100:
            file = self.fileroot + "00" + str(self.index)
        elif self.index < 1000:
            file = self.fileroot + "0" + str(self.index)
        else:
            file = self.fileroot + str(self.index)
