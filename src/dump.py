...
class dump:
    ...
    def __init__(self, *input, **kwargs):
        ...
        if isinstance(
                input[0],
                dict):  # multiprocessing code (the [0] comes from the asteriks in the argumentlist)
            dictionary = input[0]

            # check whether to output or not
            if "debugMode" in dictionary:
                outputfl = dictionary["debugMode"]
    ...
    # --------------------------------------------------------------------
    # values in old column are spread as ints from 1-N and assigned to new
    # column
    ...
    def viz(self, index, flag=0):
        ...
        if self.bondflag:
            if self.bondflag == 1:
                ...
                tmp1, tmp2, tmp3, bondlist, tmp4, tmp5 = self.objextra.viz(
                    time, 1)
            alist = {}
        ...
        if self.lineflag:
            if self.lineflag == 1:
                ...
                tmp1, tmp2, tmp3, tmp4, tmp5, lines = self.objextra.viz(
                    time, 1)
...
class tselect:
    ...
    def all(self, **kwargs):
        ...
        if outputfl:
            print(
                "%d snapshots selected out of %d" %
                (data.nselect, data.nsnaps))
