...
def particleGran(file, atoms, names, n_values):
    ...
    print("POINTS %d float" % len(atoms), file=f)
    for atom in atoms:
        # atom[3],atom[4],atom[5]  #write x,y,z  [atom[0]=id, atom[1]=type]
        print(atom[vectors['x']], atom[vectors['x'] + 1],
              atom[vectors['x'] + 2], file=f)
    print("VERTICES", len(atoms), 2 * len(atoms), file=f)
    for i in range(len(atoms)):
        print(1, i, file=f)
    print("POINT_DATA", len(atoms), file=f)
    ...
    for key in list(vectors.keys()):
        ...
        if atoms != []:
            ...
            if 'float' in vectortype:
                vectortype = 'float'
            elif 'int' in vectortype:
                vectortype = 'int'
            else:
                vectortype = 'float'
        ...
        for atom in atoms:
            print(atom[vectors[key]], atom[vectors[key] + 1],
    ...
    for key in list(scalars.keys()):
        ...
        if atoms != []:
            ...
            if 'float' in scalartype:
                scalartype = 'float'
            elif 'int' in scalartype:
                scalartype = 'int'
            else:
                scalartype = 'int'
        else:  # if no atoms are present
...
def findScalarsAndVectors(names):
    ...
    # fill missing indices (occurrs e.g. if output is like vx vy vz fx fy fz
    # vx vy vz)
    ...
    # loop over all indices and look if their names represent a vector (if
    # not: it's a scalar)
    i = 0
    while i <= max(indices):

        if i + 2 <= max(indices) and regvx.match(indices[i]) is not None and regvy.match(
                indices[i + 1]) is not None and regvz.match(indices[i + 2]) is not None:
        ...
        if regf.match(
            indices[i]) is not None or regc.match(
            indices[i]) is not None or regv.match(
                indices[i]) is not None:
            ...
            if i + \
                    2 <= max(indices) and indices[i + 1] == nextName and indices[i + 2] == nextButOneName:
                vectors[newname] = i
                i += 3
                continue
