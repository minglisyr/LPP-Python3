...
class cdata:
    ...
    def read(self, *list):
        ...
        for file in flist:
            ...
            while 1:
                ...
                if not line:
                    continue
                elif line.find("triangles") == 0:
                    flag = "triangles"
                elif line.find("particles") == 0:
                    flag = "particles"
                elif line.find("facets") == 0:
                    flag = "facets"
    ...
    def surfselect(self, id, id_surf, teststr):
        ...
        # 3 vertices must satisfy all 3 tests for tri's inclusion in new surf
        # obj
        ...
        for tri in o.triangles:
            ...
            if flag1 and flag2 and flag3:
                ...
                obj.triangles.append(
                    [obj.nvert + 1, obj.nvert + 2, obj.nvert + 3])
                obj.nvert += 3
                obj.ntri += 1
    ...
    def part(self, id, npart, in_id, out_id=None):
        ...
        # pre-process SURFACE objects to bin their triangles for faster
        # searching
    ...
    def project(self, id, id2, dx, dy, dz, EPS, flag=None):
        ...
        for i in range(obj.npart):
            ...
            while scale < maxscale:
                xnew, ynew, znew = x + scale * \
                    dir[0], y + scale * dir[1], z + scale * dir[2]
            ...
            if bracket == start:
                raise Exception(
                    "Could not find bracket point for particle %d" %
                    i)
    ...
    def center(self, id, x, y, z):
        ...
        if obj.style != SURFACE and obj.style != GROUP:
            raise Exception(
                "Can only use center() on a surface or group object")
        obj.center(x, y, z)
    ...
    def trans(self, id, dx, dy, dz):
        ...
        if obj.style != SURFACE and obj.style != GROUP:
            raise Exception(
                "Can only use trans() on a surface or group object")
        obj.xc += dx
        obj.yc += dy
        obj.zc += dz
    ...
    def rotate(self, id, axis1, i1, j1, k1, axis2, i2, j2, k2):
        ...
        if obj.style != SURFACE and obj.style != GROUP:
            raise Exception(
                "Can only use rotate() on a surface or group object")
        ...
        if axis1 == 'x':
            ...
            raise Exception("Illegal rotate arguments")
        if axis2 == 'x':
            xnew = [i2, j2, k2]
        elif axis2 == 'y':
            ynew = [i2, j2, k2]
        ...
        # apply rotation matrix of direction cosines to each vertex or part
        # coord
    ...
    def scale(self, id, sx, sy, sz):
        ...
        if obj.style != SURFACE and obj.style != GROUP:
            raise Exception(
                "Can only use scale() on a surface or group object")
        if obj.style == SURFACE:
            for i in range(obj.nvert):
                obj.vertices[i][0] = obj.xc + sx * \
                    (obj.vertices[i][0] - obj.xc)
                obj.vertices[i][1] = obj.yc + sy * \
                    (obj.vertices[i][1] - obj.yc)
                obj.vertices[i][2] = obj.zc + sz * \
                    (obj.vertices[i][2] - obj.zc)
        elif obj.style == GROUP:
            for i in range(obj.npart):
    ...
    def join(self, id, *list):
        ...
        for id in list:
            ...
            # increment vertex and triangle indices b/c now have previous
            # surfaces
        ...
        for index in vlist:
            ...
            if obj.style == GROUP:
                if not obj.on_id:
                    ...
                    print(
                        "particles %s %d %s" %
                        (obj.id, obj.npart, obj.on_id), file=fp)
                print(file=fp)
                for i in range(obj.npart):
                    print(
                        i + 1,
                ...
                print(file=fp)
            if obj.style == SURFACE:
                print("triangles %s %d %d" %
                      (obj.id, obj.nvert, obj.ntri), file=fp)
                print(file=fp)
                for i in range(obj.nvert):
...
class Surface:
    ...
    def inside(self, x, y, z):
        ...
        for m in range(n):
            ...
            if x < v1[0] and x < v2[0] and x < v3[0]:
                continue
            if x > v1[0] and x > v2[0] and x > v3[0]:
                continue
            if y < v1[1] and y < v2[1] and y < v3[1]:
                continue
            if y > v1[1] and y > v2[1] and y > v3[1]:
                continue
    ...
    def area(self):
        ...
        for tri in self.triangles:
            ...
            v2 = self.vertices[tri[1] - 1]
            v3 = self.vertices[tri[2] - 1]
            a[0] = (v2[1] - v1[1]) * (v3[2] - v1[2]) - \
                (v2[2] - v1[2]) * (v3[1] - v1[1])
            a[1] = (v2[2] - v1[2]) * (v3[0] - v1[0]) - \
                (v2[0] - v1[0]) * (v3[2] - v1[2])
            a[2] = (v2[0] - v1[0]) * (v3[1] - v1[1]) - \
                (v2[1] - v1[1]) * (v3[0] - v1[0])
...
class Box:
    ...
    def loc2d(self, area, random):
        ...
        if area <= self.areas[0]:
            return [self.xlo, self.ylo + r1 * ysize,
                    self.zlo + r2 * zsize], [-1, 0, 0]
        elif area <= self.areas[1]:
            return [self.xhi, self.ylo + r1 * ysize,
                    self.zlo + r2 * zsize], [1, 0, 0]
        elif area <= self.areas[2]:
            return [self.xlo + r1 * xsize, self.ylo,
                    self.zlo + r2 * zsize], [0, -1, 0]
        elif area <= self.areas[3]:
            return [self.xlo + r1 * xsize, self.yhi,
                    self.zlo + r2 * zsize], [0, 1, 0]
        elif area <= self.areas[4]:
            return [self.xlo + r1 * xsize, self.ylo +
                    r2 * ysize, self.zlo], [0, 0, -1]
        else:
            return [self.xlo + r1 * xsize, self.ylo +
                    r2 * ysize, self.zhi], [0, 0, 1]

    # ChemCell text to create the region

    def command(self):
        return "%s box %g %g %g %g %g %g" % (
            self.id, self.xlo, self.ylo, self.zlo, self.xhi, self.yhi, self.zhi)
...
class Sphere:
    ...
    def loc2d(self, area, random):
        ...
        c = [x, y, z]
        normalize(c)
        return [self.x + self.r * c[0], self.y +
                self.r * c[1], self.z + self.r * c[2]], c

    # ChemCell text to create the region

    def command(self):
        return "%s sphere %g %g %g %g" % (
            self.id, self.x, self.y, self.z, self.r)
        ...
        if area <= self.areas[0]:
            while 1:
                ...
                if r1 * r1 + r2 * r2 < 0.25:
                    break
            if self.axis == 'x':
                return [self.lo, self.c1 + 2 * r1 * self.r,
                        self.c2 + 2 * r2 * self.r], [-1, 0, 0]
            elif self.axis == 'y':
                return [self.c1 + 2 * r1 * self.r, self.lo,
                        self.c2 + 2 * r2 * self.r], [0, -1, 0]
            elif self.axis == 'z':
                return [self.c1 + 2 * r1 * self.r, self.c2 +
                        2 * r2 * self.r, self.lo], [0, 0, -1]
        elif area <= self.areas[1]:
            while 1:
                r1 = random() - 0.5
                r2 = random() - 0.5
                if r1 * r1 + r2 * r2 < 0.25:
                    break
            if self.axis == 'x':
                return [self.hi, self.c1 + 2 * r1 * self.r,
                        self.c2 + 2 * r2 * self.r], [1, 0, 0]
            elif self.axis == 'y':
                return [self.c1 + 2 * r1 * self.r, self.hi,
                        self.c2 + 2 * r2 * self.r], [0, 1, 0]
            elif self.axis == 'z':
                return [self.c1 + 2 * r1 * self.r, self.c2 +
                        2 * r2 * self.r, self.hi], [0, 0, 1]
        else:
            r1 = 2 * pi * random()
            r2 = random()
            if self.axis == 'x':
...
class Capped:
    ...
    def inside(self, x, y, z):
        ...
        if d3 >= self.lo and d3 <= self.hi:
            ...
            if d1 * d1 + d2 * d2 + (d3 - self.lo) * (d3 - self.lo) > self.rsq:
                return 0
        else:
            if d1 * d1 + d2 * d2 + (d3 - self.hi) * (d3 - self.hi) > self.rsq:
                return 0
        return 1
    ...
    def triangulate(self):
        ...
        vertices, triangles = box_triangulate(
            self.q1, self.q1, self.q2 + self.q1)
        self.nvert = len(vertices)
        self.ntri = len(triangles)
        self.vertices = []
        ...
        for i in range(self.nvert):
            ...
            if v3 < cutlo:
                ...
                p1 = self.c1 + self.r * c[0]
                p2 = self.c2 + self.r * c[1]
                p3 = self.lo + (v3 - cutlo) / (cuthi - cutlo) * \
                    (self.hi - self.lo)
            if self.axis == 'x':
                self.vertices.append([p3, p1, p2])
            elif self.axis == 'y':
    ...
    def loc2d(self, area, random):
        if area <= self.areas[0]:
            while 1:
                ...
                r2 = random() - 0.5
                r3 = random() - 0.5
                if r3 <= 0 and r1 * r1 + r2 * r2 + r3 * r3 < 0.25:
                    break
            c = [r1, r2, r3]
            normalize(c)
            if self.axis == 'x':
