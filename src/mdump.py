...
class mdump:
    ...
    def read_snapshot(self, f):
        try:
            ...
            str = f.readline()
            if "NUMBER OF NODES" in str:
                snap.nflag = 1
            elif "NUMBER OF TRIANGLES" in str:
                snap.eflag = 1
            elif "NUMBER OF TETS" in str:
                snap.eflag = 2
            elif "NUMBER OF SQUARES" in str:
                snap.eflag = 3
            elif "NUMBER OF CUBES" in str:
                snap.eflag = 4
            elif "NUMBER OF NODE VALUES" in str:
                snap.nvalueflag = 1
            elif "NUMBER OF ELEMENT VALUES" in str:
                snap.evalueflag = 1
            else:
                raise Exception("unrecognized snapshot in dump file")
            n = int(f.readline())

            if snap.eflag:
                snap.eselect = np.zeros(n)
    ...
    def reference(self):
        for i in range(len(self.snaps)):
            ...
            if not self.snaps[i].eflag:
                raise Exception(
                    "no elem connections found in previous snapshots")
    ...
    def viz(self, index, flag=0):
        ...
        if self.etype == "":
            ...
            type = self.names[self.etype]

        atoms = []
        bonds = []

        # create triangle list from all elements
        # for type, either use element type (-1) or user-defined column in
        # evalues
        ...
        for i in range(snap.nelements):
            ...
            if snap.eflag == 1:
                ...
                if type == -1:
                    tris.append([element[0], element[1]] + list + n)
                else:
                    tris.append([element[0], evalue[type]] + list + n)
    ...
    def mviz(self, index, flag=0):
        ...
        nvalues = []
        if snap.nvalueflag:
            nvalues = snap.nvalues
        evalues = []
        if snap.nvalueflag:
            evalues = snap.evalues
    ...
    def maxbox(self):
        ...
        for snap in self.snaps:
            if not snap.tselect:
                continue
            if xlo is None or snap.xlo < xlo:
                xlo = snap.xlo
            if xhi is None or snap.xhi > xhi:
                xhi = snap.xhi
            if ylo is None or snap.ylo < ylo:
                ylo = snap.ylo
            if yhi is None or snap.yhi > yhi:
                yhi = snap.yhi
            if zlo is None or snap.zlo < zlo:
                zlo = snap.zlo
            if zhi is None or snap.zhi > zhi:
                zhi = snap.zhi
