...
try:
    from DEFAULTS import PIZZA_GNUTERM
except BaseException:
    PIZZA_GNUTERM = "x11"
...
class gnu:
    ...
    def plot(self, *vectors):
        if len(vectors) == 1:
            ...
            if len(vectors) % 2:
                raise Exception("vectors must come in pairs")
    ...
    def save(self, file):
        self.__call__(
            "set terminal postscript enhanced solid lw 2 color portrait")
    ...
    def draw(self):
        ...
        if fig.xlog:
            self.__call__("set logscale x")
        else:
            self.__call__("unset logscale x")
        if fig.ylog:
            self.__call__("set logscale y")
        else:
            self.__call__("unset logscale y")
        if fig.xlimit:
            cmd = 'set xr [' + str(fig.xlimit[0]) + ':' + \
                str(fig.xlimit[1]) + ']'
            self.__call__(cmd)
        else:
            self.__call__("set xr [*:*]")
        if fig.ylimit:
            cmd = 'set yr [' + str(fig.ylimit[0]) + ':' + \
                str(fig.ylimit[1]) + ']'
            self.__call__(cmd)
        else:
        ...
        for i in range(fig.ncurves):
            ...
            if len(fig.colors) > i and fig.colors[i]:
                cmd += "'" + file + \
                    "' using 1:2 with line %d, " % fig.colors[i]
            else:
